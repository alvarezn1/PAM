...
export class VistaIngresosPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userId) {
      this.getIngreso(); // Solo llama a getGastos si userId no es null
    } else {
  ...
  getIngreso() {
    ...
    this.db.list(`usuarios/${this.userId}/ingresos`).snapshotChanges().subscribe(
      actions => {
        ...
        console.log('Gastos recuperados:', this.ingresos); // Para depuración
      },
      error => {
        console.error('Error al recuperar los ingresos:', error); // Para manejar errores de la base de datos
      }
    );
  }
